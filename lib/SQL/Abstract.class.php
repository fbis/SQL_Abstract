...
/**
* SQL_Abstract
* CPANのSQL::AbstractのPHP版です。PHP4でもPHP5でも動作可能です。
* 
...
*                     ソースコードをUTF8に変更
* 2009/07/15 ver 0.03 <>に対応
* 2009/07/22 ver 0.04 insertとupdateの-injectに対応
* 2009/11/11 ver 0.05 array_key_existsを可能な限りissetへ変更。
* 2010/02/21 ver 0.06 order byを配列じゃなくても渡せるように変更
* 2010/06/28 ver 0.07 group byを渡せるようにした
* 
* @version 0.07
* @package SQL
* @access public
...
class SQL_Abstract {
    ...
    */
    ...
    function select ($table,$columns,$where,$order=null,$group=null) {
    ...
    function insert ($table,$fieldvals) {
        ...
        foreach ($fieldvals as $col => $val) {
            $columns .=  $col.',';
            if ( is_array($val) && isset($val['-inject']) ){
                $values  .=  $val['-inject'].',';
            }
            else {
                $values  .=  '?,';
    ...
    function update ($table,$fieldvals,$where=array()) {
        $set     = array();
        $params  = array();
    ...
    * @param array GROUP句(省略可)
    * @return 連想配列で且つキーが-and,-orならその文字列を返す。
    *         連想配列なら真を返す。
    *         配列なら偽を返す。
    */
    function where ($where,$order=null,$group=null){
    ...
    function _where (&$column,&$colarray,$mode=' OR ') {
        ...
        foreach( $colarray as $ope => $val ){
            ...
            if( is_null($val) ) {
                $sqls[] = ( $ope === '!=' || $ope === '<>' ) ? "$column IS NOT NULL" : "$column IS NULL";
                continue;
            }
